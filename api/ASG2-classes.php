...
ImageFileName, Title, YearOfWork, Excerpt, Paintings.Description, Width, Height,
Medium, CopyrightText, WikiLink, MuseumLink, JsonAnnotations FROM Galleries INNER JOIN (Artists
...
Galleries.GalleryID = Paintings.GalleryID";
    ...
    {
        $sql = self::$baseSQL . " ORDER BY Paintings.YearOfWork";
        $statement = DatabaseHelper::runQuery(
            $this->pdo,
            $sql,
            null
    ...
    public function getAllForTitle()
    {
        $sql = self::$baseSQL . " ORDER BY Paintings.Title";
        $statement = DatabaseHelper::runQuery(
            $this->pdo,
            $sql,
            null
    ...
    {
        $limitSQL = self::$baseSQL . " ORDER BY YearOfWork LIMIT 20";
        $statement = DatabaseHelper::runQuery(
            $this->pdo,
            $limitSQL,
            null
...
class GalleryDB
{
    private static $baseSQL = "SELECT GalleryID, GalleryName, GalleryNativeName, GalleryCity,
    GalleryAddress, GalleryCountry, Latitude, Longitude, GalleryWebSite, GooglePlaceID from Galleries";
